...
const vertexShader = `
  ...
  void main() {
    ...
    // Rotate sphere
    float rotY = uTime * 0.15;
    float rotX = uTime * 0.08;
    mat2 rotYMat = mat2(cos(rotY), -sin(rotY), sin(rotY), cos(rotY));
    mat2 rotXMat = mat2(cos(rotX), -sin(rotX), sin(rotX), cos(rotX));
    
    pos.xz = rotYMat * pos.xz;
    pos.yz = rotXMat * pos.yz;
    ...
    pos.y += cnoise(noisePos + vec3(10.0)) * noiseAmp;
    pos.z += cnoise(noisePos + vec3(20.0)) * noiseAmp;
    ...
    if(dist < interactionRadius) {
        vec3 dir = pos - uMouse;
        // Add some z-push to make it look 3D
        dir.z += 1.0;
        dir = normalize(dir);
    ...
    gl_PointSize = (1.5 * aRandom + 1.0) * (15.0 / -mvPosition.z);
