...
const startIdx = content.indexOf('const vertexShader = `');
const endIdx = content.indexOf('const WORDS = ["scale", "elevate", "integrate", "systemize"];');
...
const newContent = `const noiseShaderChunk = \`
...
const pointsFragmentShader = \`
  ...
  void main() {
    ...
    // Subtle depth fading
    float depth = (vPosition.z + 1.0) * 0.5; // 0 to 1
    float alpha = mix(0.1, 0.8, depth);
...
const wireframeFragmentShader = \`
  ...
  void main() {
    ...
    float alpha = mix(0.02, 0.15, depth);
    gl_FragColor = vec4(0.1, 0.1, 0.1, alpha);
  }
\`;
...
function ParticleSphere() {
  ...
  // Adjusted scale: increased by 20% from previous
  const baseScale = size.width < 868 ? 2.16 : 3.12;
...
console.log("Successfully replaced ParticleSphere");
